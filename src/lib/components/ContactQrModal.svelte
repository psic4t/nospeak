<script lang="ts">
    ...
    async function loadProfileAndQr() {
        ...
        try {
            ...
            if (profile?.metadata) {
                displayName = resolveDisplayName(profile.metadata, npub);
                picture = profile.metadata.picture;
            } else {
                displayName = resolveDisplayName(null, npub);
                picture = undefined;
            }
            ...
            console.error('Failed to prepare contact QR modal:', e);
        } finally {
            loading = false;
        }
    }
...
{#if isOpen}
    <div
        ...
        class="fixed inset-0 bg-black/35 md:bg-black/40 bg-gradient-to-br from-black/40 via-black/35 to-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[60] p-4"
